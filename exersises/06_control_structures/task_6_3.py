...
for tintf, tvlan in fast_int['trunk'].items():
  print('\ninterface FastEthernet' + tintf)
  for tcommand in trunk_template:
    if tcommand.endswith('allowed vlan'):
      if tvlan[0] == 'add':
        itvlan = str(tvlan[1:])
        itvlan = itvlan.strip('[]').replace("'", "")
        print(' {} add {}'.format(tcommand, itvlan))
      if tvlan[0] == 'del':
        itvlan = str(tvlan[1:])
        itvlan = itvlan.strip('[]').replace("'", "")
        print(' {} remove {}'.format(tcommand, itvlan))
      if tvlan[0] == 'only':
        itvlan = str(tvlan[1:])
        itvlan = itvlan.strip('[]').replace("'", "")
        print(' {} {}'.format(tcommand, itvlan))
      else:
        pass
    else:
